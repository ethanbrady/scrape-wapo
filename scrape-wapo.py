...
def soupify():
    ...
    main_url = 'https://www.washingtonpost.com'
    response1 = requests.get(main_url)
...
def format_elements():
    ...
    story_soup, title, subtitle, authors, publish_date, modify_date, image_url = soupify()
    elements = [title.upper(),
                '**' + subtitle + '**',
                'By ' + parse_authors(authors),
                parse_date(publish_date).strftime('%b. %d, %Y'),
                'Updated ' + parse_date(modify_date).strftime('%-H:%M')
                ]
